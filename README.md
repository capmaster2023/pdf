...
## Ce qui est livré dans la v0.1.0 (fonctionnel, rien de factice)
...
3. Annotations (surlignage, notes, dessin)
4. Signature manuscrite
5. Compression / optimisation
6. Protection et déverrouillage par mot de passe
7. Rotation / suppression / réorganisation de pages
8. Filigrane, numérotation
9. Hilt (le projet utilise pour l'instant une injection manuelle simple — `AppContainer` — choisie pour garantir une compilation fiable dès la première version ; la migration est directe)
...
Puis pousse un tag : `git tag v0.1.0 && git push origin v0.1.0` → workflow **Build Release APK**. Sans secrets, la release sort non signée.
...
├── pdf/PdfToolbox.kt      Fusion + extraction (PDFBox), images→PDF (API système)
├── navigation/            Routes + Scaffold + barre inférieure
├── ui/theme/              Material 3, couleurs dynamiques (Android 12+)
└── features/              home, files, viewer, tools (merge/split/images), settings
```
